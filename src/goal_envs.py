...
class RotaryInvertedDoublePendulumGoalEnv(InvertedDoublePendulumEnv):
    def __init__(self, mode="test", custom_xml_file=RDIP_XML_DIR, *args, **kwargs):
        ...
        self.observation_space = spaces.Dict({
            ...
            "achieved_goal": spaces.Box(low=-np.pi, high=np.pi, shape=(goal_dim,), dtype=np.float32),
            "desired_goal": spaces.Box(low=-np.pi, high=np.pi, shape=(goal_dim,), dtype=np.float32),
        })
