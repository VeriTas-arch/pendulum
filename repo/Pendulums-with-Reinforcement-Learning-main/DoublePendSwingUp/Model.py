...
class SACAgent:
    ...
    def __init__(self, env, load=False):
        ...
        if load is True:
            t = input("Latest Model Time (HH_MM): ")
            current_path = Path(__file__).parent.resolve()
            PATH = f"{current_path}/saved_models/{t}/"
            self.actor.load_state_dict(
                torch.load(PATH + "actor.pth", map_location=torch.device("xpu"))
            )
            self.critic_1.load_state_dict(
                torch.load(PATH + "critic_1.pth", map_location=torch.device("xpu"))
            )
            self.critic_2.load_state_dict(
                torch.load(PATH + "critic_2.pth", map_location=torch.device("xpu"))
            )
            self.target_critic_1.load_state_dict(
                torch.load(
                    PATH + "target_critic_1.pth", map_location=torch.device("xpu")
                )
            )
            self.target_critic_2.load_state_dict(
                torch.load(
                    PATH + "target_critic_2.pth", map_location=torch.device("xpu")
                )
            )
        ...
        for ep in tqdm(range(max_episode_num)):
            ...
            while not truncated and not terminated and time < 500:
                ...
                if self.buffer.count > 100:
                    ...
                    self.critic_learn(
                        torch.tensor(states).to(torch.float32).to(self.actor.device),
                        torch.tensor(actions).to(torch.float32).to(self.actor.device),
