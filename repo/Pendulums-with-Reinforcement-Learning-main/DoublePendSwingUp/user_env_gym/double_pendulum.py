    ...
    **Note:** While the ranges above denote the possible values for observation space of each element,
    ...
    def __init__(self, render_mode: Optional[str] = None, reward_mode: int = 0):
        ...
        self.action_space = spaces.Box(
            -self.torque_mag, self.torque_mag, dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        #     -self.torque_mag, self.torque_mag
        # )
        ...
        def pend(y, t, tau, m_1, m_2, l_1, l_2, g, b_1=0, b_2=0):
            ...
            th2_2dot = tmp1 - (
                3 * l_1 * cosdelth * (tmp2 + 0.5 * m_2 * l_1 * l_2 * tmp1 * cosdelth)
            ) / (2 * l_2 * tmp3)
        ...
        if not terminated:
            ...
            if self.reward_mode == 1:
                ...
                reward = (
                    0.3
                    * math.sqrt(2 * np.pi)
                    * norm_dist_th1.pdf(np.pi - self.state[0])
                    - 0.5
                )
            ...
            if self.reward_mode == 3:
                reward = (
                    -((np.pi - self.state[0]) ** 2)
                    - 0.1 * (np.pi - self.state[2]) ** 2
                    - 0.001 * torque**2
                )
                if self.prev_state is not None:
                    reward -= (
                        0.5 * ((self.state[0] - self.prev_state[0]) / self.tau) ** 2
                        + 0.05 * ((self.state[2] - self.prev_state[2]) / self.tau) ** 2
                    )

                if isinstance(reward, float) is False:
                    reward = reward.squeeze()

            if self.reward_mode == 4:
                in_bound = bool(
                    np.radians(135) < theta1 < np.radians(135 + 90)
                    and np.radians(135) < theta2 < np.radians(135 + 90)
                )
                if in_bound:
                    reward = 1
                else:
        ...
        if self.render_mode == "human":
            self.render()
        return (
            np.array(self.outputstate, dtype=np.float32),
            reward,
            terminated,
            False,
            {},
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self.state = self.np_random.uniform(
            low=[0.0, -0.5, 0.0, -0.5], high=[2 * np.pi, 0.5, 2 * np.pi, 0.5], size=(4,)
        )
        self.steps_beyond_terminated = None
        self.prev_state = None
        self.episode_length = 0
    ...
    def render(self):
        ...
        l, r, t, b = (-polewidth / 2, polewidth / 2, polelen1, -polewidth / 2)

        pole_coords = []
        for coord in [(l, b), (l, t), (r, t), (r, b)]:
            coord = pygame.math.Vector2(coord).rotate_rad(
                x[0] + np.pi
            )  # 여기 좌표 체계가 잘 이해되지 않는다..
            coord = (
                coord[0] + self.screen_width / 2,
                coord[1] + self.screen_height / 2,
            )
            pole_coords.append(coord)
        gfxdraw.aapolygon(self.surf, pole_coords, (202, 152, 101))
        gfxdraw.filled_polygon(self.surf, pole_coords, (202, 152, 101))

        l, r, t, b = (-polewidth / 2, polewidth / 2, polelen2, -polewidth / 2)

        pole_coords = []
        for coord in [(l, b), (l, t), (r, t), (r, b)]:
            coord = pygame.math.Vector2(coord).rotate_rad(x[2] + np.pi)
            coord = (
                coord[0] + self.screen_width / 2 + polelen1 * np.sin(x[0]),
                coord[1] + self.screen_height / 2 - polelen1 * np.cos(x[0]),
            )
