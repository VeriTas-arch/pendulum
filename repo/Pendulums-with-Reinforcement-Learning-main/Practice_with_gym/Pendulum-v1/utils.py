...
class ExperienceReplay:
    ...
    def sample(self):
        ...
        action_batch = (
            torch.Tensor(np.array([a for (s1, a, r, s2, d) in minibatch]))
            .view(-1, 1)
            .to(device)
        )
        reward_batch = (
            torch.Tensor([r for (s1, a, r, s2, d) in minibatch]).view(-1, 1).to(device)
        )
        next_state_batch = torch.cat(
            [s2.view(1, -1) for (s1, a, r, s2, d) in minibatch]
        ).to(device)
        done_batch = (
            torch.Tensor([d for (s1, a, r, s2, d) in minibatch]).view(-1, 1).to(device)
        )
