...
from dataModels import Config, DDPGParams, DQNParams, ModelParams
...
class DQNAgent:
    ...
    def __init__(self, config: ModelParams) -> None:
        ...
        self.state_shape = (
            5,
        )  # State shape (x_pos, x_vel, sin(angle), cos(angle), angle_vel)
        self.action_size = (
            config.DQN.num_actions
        )  # Number of actions (2 for left and right, 3 for left, right and no action)

        self.memory = deque(
            maxlen=config.memory_size
        )  # Circular buffer used to store transitions to learn from.
        self.gamma = config.discount_factor  # Discount rate
        self.eps_init = (
            config.DQN.epsilon.epsilon_init
        ...
        )  # Magnitude of the force applied to the base
    ...
    def build_model(self, config: DQNParams):
        ...
        )  # Input layer with 5 neurons (state representation)
        for layer_size in config.hidden_layer_sizes[1:]:
            model.add(
                Dense(layer_size, activation="relu")
            )  # Hidden layers with specified number of neurons
        model.add(
            Dense(self.action_size, activation="linear")
        )  # Output layer with 2 neurons (one for each action)
        model.compile(
            loss="mse", optimizer=Adam(learning_rate=config.learning_rate)
    ...
    def act(self, state):
        ...
        force = (
            -self.force_magnitude if action == 0 else self.force_magnitude
        )  # Convert action to force value
    ...
    def update(self, batch_size):
        ...
        for i, (state, action, reward, next_state, done) in enumerate(minibatch):
            ...
            target_f[0][
                action
            ] = target  # Update the estimate of future return for the action taken
    ...
    def train(self, env: Pendulum, config: Config):
        ...
        for e in range(num_episodes):
            ...
            while not done:  # Play episode
                ...
                if done:
                    print(
                        f"episode: {e}/{num_episodes}, score: {time_step}, eps: {self.eps_init:.2}"
                    )
                    break
            ...
            if config.model.IO_parameters.save_weights.enable:
                if (
                    e % config.model.IO_parameters.save_weights.save_frequency == 0
                    and e != 0
                ):
...
class REINFORCEAgent:
    ...
    def __init__(self, config: ModelParams) -> None:
        ...
        self.action_size = (
            config.REINFORCE.num_actions
        )  # Number of actions (2 for left and right, 3 for left, right and no action)
        self.hidden_layer_sizes = (
            config.REINFORCE.hidden_layer_sizes
        ...
        self.memory = deque(
            maxlen=config.memory_size
        )  # Used to store transitions to learn from.
        self.force_magnitude = (
            config.force_magnitude
        )  # Magnitude of the force applied to the base
        self.gamma = config.discount_factor  # Discount rate
    ...
    def custom_loss(self, y_true, y_pred):
        ...
        log_probs = y_true * tf.math.log(
            y_pred_clipped
        )  # Log(pi(s)) * G*gamma^t (one-hot-encoded for the selected action)
    ...
    def build_model(self):
        ...
        model.compile(
            loss=self.custom_loss, optimizer=Adam(learning_rate=self.learning_rate)
        )
        return model
    ...
    def act(self, state):
        ...
        action = np.random.choice(
            np.arange(len(act_probs)), p=act_probs
    ...
    def update(self, batch_size):
        ...
        print("Running replay...")
        states = np.zeros(
            (len(minibatch), self.state_shape[0])
        )  # Create array to store states
        y_true = np.zeros(
            (len(minibatch), self.action_size)
        )  # Create array to store y_true
        for i, (state, action, reward, G, time_step) in enumerate(
            minibatch
        ):  # Loop over mini-batch
            states[i] = state  # Add state to states
            y_true[i][action] = G * tf.math.pow(
                self.gamma, time_step
            )  # Multiply G*gamma^t with one-hot-encoded action
    ...
    def train(self, env: Pendulum, config: Config):
        ...
        for e in range(num_episodes):  # Loop over episodes
            ...
            while not done:  # Play episode
                ...
                ep.append(
                    (state, action, reward)
                )  # Add state, action, reward to episode history
                if done:
                    print(
                        f"episode: {e}/{num_episodes}, score: {R}, advantage: {R}, steps: {time_step}"
                    )
                    break
            ...
            for i, r in enumerate(R[::-1]):
                ...
                ep[i] = (
                    ...
                    G,
                    i,
                )  # Add G and time step to episode history
...
class DDPGAgent:
    ...
    def __init__(self, config: ModelParams) -> None:
        ...
        self.polyak_critic = (
            config.DDPG.critic.polyak
        )  # Polyak averaging parameter for critic target network
        self.polyak_actor = (
            config.DDPG.actor.polyak
        )  # Polyak averaging parameter for actor target network

        self.force_magnitude = config.force_magnitude

        self.actor_model, self.critic_model = self.build_models(config=config.DDPG)
        self.target_actor_model, self.target_critic_model = self.build_models(
            config=config.DDPG
        )
        if config.IO_parameters.init_from_weights.enable:
            self.load_weights(config.IO_parameters.init_from_weights.file_path)
        ...
        self.noise_scaling = (
            config.DDPG.actor.ornstein_uhlenbeck_noise.noise_scale_initial
        )
        self.noise = OrnsteinUhlenbeckNoise(
            mu=np.zeros(1),
            sigma=config.DDPG.actor.ornstein_uhlenbeck_noise.sigma,
            theta=config.DDPG.actor.ornstein_uhlenbeck_noise.theta,
            dt=0.05,
        )
    ...
    def build_models(self, config: DDPGParams):
        ...
        output = Dense(
            self.action_size, activation="tanh", kernel_initializer=output_init
        )(h)
        ...
        # State input layers
        state_input = Input(shape=self.state_shape)
        state_h = Dense(config.critic.state_input_layer_sizes[0], activation="relu")(
            state_input
        )
        ...
        action_h = Dense(config.critic.action_input_layer_sizes[0], activation="relu")(
            action_input
        )
    ...
    def update(self):
        ...
        future_qs = self.target_critic_model.predict(
            [next_states, target_actions], verbose=0
        ).flatten()
        target_qs = rewards + self.gamma * future_qs * (
            1 - dones
        )  # Handle terminal states with (1 - dones)
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            critic_value = self.critic_model(
                [states, actions_for_training], training=True
            )
        ...
        self.update_target(
            self.target_critic_model, self.critic_model, self.polyak_critic
        )
    ...
    def train(self, env: Pendulum, config: Config):
        ...
        for e in range(num_episodes):
            ...
            while not done:
                ...
                if done:
                    print(
                        f"episode: {e}/{num_episodes}, score: {episode_reward}, steps: {time_step}, noise: {self.noise_scaling:.2}"
                    )
                    break
                self.update()
                state = next_state
            ...
            # Decay noise scaling
            self.noise_scaling *= (
                config.model.DDPG.actor.ornstein_uhlenbeck_noise.noise_decay
            )
