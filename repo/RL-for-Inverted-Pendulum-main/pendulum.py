...
class Pendulum:
    ...
    The pendulum consists of a massless rod of a certain length with a bob at
    the end. The base of the pendulum can be moved horizontally by applying a
    force. The goal is to balance the pendulum in upright position by moving
    the base.
    """
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.x_lim = (
            config.pendulum.termination.x_limit
        )  # x limit for the pendulum (in meters). Set to None for no limit.
        self.angle_lim = (
            None
            if config.pendulum.termination.angle_limit is None
            else config.pendulum.termination.angle_limit * np.pi / 180
        )  # Angle limit for the pendulum (in degrees). Set to None for no limit.
        self.time_limit = (
            config.pendulum.termination.time_limit
        )  # Time limit for the episode in seconds. Set to None for no limit.
        self.state = (
            self.reset()
        )  # State vector [x_pos, y_pos, x_vel, y_vel, angle, ang_vel]
        self.terminated = False  # Termination flag
        self.termination_penalty = (
            config.pendulum.termination.termination_penalty
        ...
        self.episode_log = (
            []
        )  # Log of the episode. It will contain pairs of (state, force) for each time step in the episode.
        self.logging_enable = config.pendulum.logging.enable  # Enable logging
        self.log_file_path = (
            config.pendulum.logging.file_path
    ...
    def derivate_state(self, state, force):
        """
        Compute the derivative of the state vector using the current state and applied force.
        ...
        )  # Unit vector in the direction of the pendulum
        fb = np.dot(force, u) * u + np.array([0, -self.g])  # Force acting on the bob
        base_acc = (
            force
        ) / self.mass_base  # Acceleration of the base (independent of the bob)
        ang_acc = (
            np.cross(force - fb, self.length * u) / (self.mass_bob * self.length**2)
            - self.config.pendulum.damping_coefficient * ang_vel
        )  # Angular acceleration of the bob
        state_dot = np.array(
            [vel[0], vel[1], base_acc[0], base_acc[1], ang_vel, ang_acc]
    ...
    def get_state_representation(self, state):
        ...
        state_repr : np.ndarray
            The processed state vector suitable for the agent as
            ``[x_pos_norm, x_vel_norm, cos(angle), sin(angle), ang_vel_norm]``.
        """
        x_pos = state[0]
        x_vel = state[2]
        angle = state[4]
        ...
        state = np.array(
            [x_pos_norm, x_vel_norm, np.cos(angle), np.sin(angle), angle_vel_norm]
        )
    ...
    def reward(self, force=None):
        ...
        The reward is computed based on the deviation from the
        goal state (i.e. upright and stationary at the center).
        ...
        angular_velocity_norm = (
            self.state[5] / 7
        )  # Normalize angular velocity to [-1, 1] (assuming max angular velocity is 7 rad/s)
        x_position = (
            self.state[0] / self.x_lim if self.x_lim is not None else self.state[0]
        )  # Normalize x position to [-1, 1] if a limit is set

        # Compute the combined deviation from the goal state
        # The goal state is upright and stationary at the center
        deviation = np.sqrt(
            (angle / np.pi) ** 2 + angular_velocity_norm**2 + x_position**2
        )
        ...
        force_penalty = (
            0
            if force is None
            else -0.01 * ((force[0] / self.config.model.force_magnitude) ** 2)
    ...
    def step(self, force):
        ...
        if self.logging_enable:
            self.episode_log.append(
                (self.state.copy(), force[0], self.reward(force))
            )  # Log the state-action pair

        self.update(force)
        reward = (
            self.reward(force) if self.config.model.type == "DDPG" else self.reward()
        )
