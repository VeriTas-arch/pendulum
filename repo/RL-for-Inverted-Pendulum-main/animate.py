...
def run_episode(env, agent):
    ...
    while not done:
        force, action = agent.act(env.get_state_representation(state))
...
def read_ep_from_log(file_path, ep_number):
    ...
    episode_data = data[
        data["ep"] == ep_number
    ]  # Extract data for the specified episode
...
def animate_episode(state_history, config: Config):
    ...
    ax.minorticks_on()
    ax.set_xticks(
        np.arange(-x_lim, x_lim + 0.5, 0.5), minor=True
    )  # Minor ticks every half meter
    ...
    base = Rectangle(
        ...
        linewidth=1.5,
        zorder=2,
    )  # Use facecolor for fill and edgecolor for border
    ax.add_patch(base)

    # Initialize pendulum line and bob with grey color and clipping
    (pendulum_line,) = ax.plot(
        [], [], "-", lw=2, color="black", zorder=1, clip_on=True
    )  # Clip pendulum line
    (pendulum_bob,) = ax.plot(
        [],
        [],
        "o",
        markersize=8,
    ...
    def update(frame):
        ...
        base.set_xy(
            (x_pos - base_width / 2, -base_height / 2)
        )  # Center the rectangle at x_pos
