...
class TerminationParams(BaseModel):
    angle_limit: Optional[float] = Field(
        None,
        description="Angle limit for the pendulum in degrees. Set to None for no limit. Should only be set if initial_state is 'up'.",
    )
    x_limit: Optional[float] = Field(
        None,
        description="X limit for the pendulum in meters. Set to None for no limit.",
    )
    time_limit: Optional[float] = Field(
        None,
        description="Time limit for the episode in seconds. Set to None for no limit.",
    )
    termination_penalty: float = Field(
        -20, description="Penalty for terminating the episode early."
    )
...
class PendulumParams(BaseModel):
    deterministic_initial_state: bool = Field(
        False, description="Use a deterministic initial state or add noise."
    )
    initial_state: Literal["up", "down", "random"] = Field(
        "up", description="Initial state of the pendulum (up, down or random)."
    )
    ...
    damping_coefficient: float = Field(
        0.1, description="Damping coefficient for the pendulum."
    )
...
class InitFromWeights(BaseModel):
    enable: bool = Field(
        False, description="Enable loading initial weights from a file."
    )
    file_path: Optional[str] = Field(
        None, description="Filename for loading initial weights."
    )
...
class SaveWeights(BaseModel):
    ...
    file_path: Optional[str] = Field(
        None, description="Filename for saving model weights."
    )
    save_frequency: Optional[int] = Field(
        50, description="Save weights every n episodes."
    )
...
class EpsilonParams(BaseModel):
    epsilon_init: float = Field(
        1, description="Initial epsilon value (for epsilon-greedy policy)."
    )
...
class DQNParams(BaseModel):
    hidden_layer_sizes: List[int] = Field(
        [16, 16], description="Number of neurons in each layer."
    )
    learning_rate: float = Field(1e-3, description="Learning rate for the optimizer.")
    num_actions: int = Field(
        2,
        description="Number of actions in the action space. (2 for left and right, 3 for left, right and no action)",
    )
    polyak: float = Field(
        0.95, description="Polyak averaging parameter for target network updates."
    )
...
class NoiseParams(BaseModel):
    enable: bool = Field(
        True, description="Enable addition of noise to action for exploration."
    )
    theta: float = Field(
        0.15, description="Theta parameter for the Ornstein-Uhlenbeck noise process."
    )
    sigma: float = Field(
        0.2, description="Sigma parameter for the Ornstein-Uhlenbeck noise process."
    )
...
class DDPGCriticParams(BaseModel):
    state_input_layer_sizes: List[int] = Field(
        [8], description="Number of neurons in layers receiving the state input."
    )
    action_input_layer_sizes: List[int] = Field(
        [8], description="Number of neurons in layers receiving the action input."
    )
    combined_layer_sizes: List[int] = Field(
        [16, 8, 8],
        description="Number of neurons in each layer for combined state and action input.",
    )
    learning_rate: float = Field(1e-3, description="Learning rate for the optimizer.")
    polyak: float = Field(
        0.95, description="Polyak averaging parameter for target network updates."
...
class ModelParams(BaseModel):
    type: Literal["DQN", "REINFORCE", "DDPG"] = Field(
        "DDPG", description="Type of model to use."
    )
    ...
    discount_factor: float = Field(
        0.95, description="Discount factor for future rewards."
    )
    force_magnitude: float = Field(
        20, description="Magnitude of the force applied to the base."
    )
...
class RunParams(BaseModel):
    type: Literal["train", "test"] = Field(
        "test", description="Run mode: 'train' or 'test'."
    )
...
class AnimateFromLogParams(BaseModel):
    enable: bool = Field(
        False,
        description="Enable animation from file, else run simulation and animate.",
    )
...
class AnimateParams(BaseModel):
    ...
    save_path: Optional[str] = Field(
        None, description="Path to save the animation. None for no save."
    )
